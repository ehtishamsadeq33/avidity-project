...
export const API_BASE = DOMAIN
  ? `https://${DOMAIN}${API_PORT ? `:${API_PORT}` : ""}/api`
  : "/api";
...
export async function adminLogin(
  ...
  return adminFetch("/admin/login", null, { method: "POST", body: { username, password } });
}

export async function adminSetup(username: string, password: string): Promise<void> {
  await adminFetch("/admin/setup", null, { method: "POST", body: { username, password } });
}
