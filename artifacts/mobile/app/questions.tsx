...
export default function QuestionsScreen() {
  ...
  const slice = useMemo(
    () => questions.slice(start, start + PAGE_SIZE),
    [questions, start],
  );

  const allAnsweredOnPage = slice.every(
    (q) => typeof answers[q.id] === "number",
  );
  const allAnsweredTotal =
    questions.length > 0 &&
    ...
    <View style={styles.root}>
      <View style={styles.headerWrap}>
        ...
        <Text style={styles.subtitle}>
          Please scale the following statements on a scale of 1 to 10 whether
          you agree or if the statement relates to you, with 1 being the lowest
        ...
        <Text style={styles.progressLabel}>
          Page {page + 1} of {totalPages} · {Object.keys(answers).length} /{" "}
